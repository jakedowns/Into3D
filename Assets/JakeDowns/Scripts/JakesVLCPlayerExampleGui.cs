...
public class JakesVLCPlayerExampleGui : MonoBehaviour
{
	...
	public GameObject trackButtonGroupScrollView; // Group containing scrolling list
	public GameObject tracksButtonsGroup; //Group containing buttons to switch video, audio, and subtitle tracks
	public GameObject tracksButtonsVerticalList;
    ...
    private const int systemMaxVolume = 15;

	public GameObject playerControlGroup;
    ...
    void Start()
	{
        ...
        playerControlGroup = GameObject.Find("Play/Stop");
        playerControlGroup.SetActive(false);
		...
		{ 
			...
			{
                Debug.LogError("Error getting AudioManager: " + e.Message);
            }
        ...
        tracksButton.onClick.AddListener(() => { ToggleElement(trackButtonGroupScrollView); SetupTrackButtons(); });
		volumeButton.onClick.AddListener(() => { ToggleElement(volumeBar.gameObject); });
		openButton.onClick.AddListener(() => {
			Debug.LogWarning("pathInputField.text: " + pathInputField.text);
		...
		*/
        ...
        trackButtonGroupScrollView.SetActive(false);

		//Volume Bar
		volumeBar.wholeNumbers = true;
		volumeBar.maxValue = maxVolume; //You can go higher than 100 but you risk audio clipping
		volumeBar.onValueChanged.AddListener((data) => { 
			vlcPlayer.SetVolume((int)volumeBar.value);
            // Set the volume of the device
            audioManager?.Call("setStreamVolume", 3, (int)((volumeBar.value/100) * systemMaxVolume), 0);
        });
	...
	void IngestARWidthHeightInput()
	{
        ...
        vlcPlayer.SetCurrentARString($"{width_rounded}:{height_rounded}");
		vlcPlayer.UpdateMaterialProps();
    ...
    void UpdateARWidthAndHeightFromCombo()
	{
		...
		vlcPlayer.SetCurrentARString($"{fraction[0]}:{fraction[1]}");
        vlcPlayer.UpdateMaterialProps();
	...
	{
		var childCount = tracksButtonsVerticalList.transform.childCount;
		for (int i = 0; i < childCount; i++)
		{
			Destroy(tracksButtonsVerticalList.transform.GetChild(i).gameObject);
		}
	}
	...
	void SetupTrackButtonsGroup(TrackType type, string label, List<Button> buttonList, bool includeNone = false)
	{
        ...
        Debug.LogWarning("SetupTrackButtonsGroup type:" + type + " tracksCount:" + tracks.Count + " selected:" + selected);

        if (tracks.Count > 0)
		{
			var newLabel = Instantiate(trackLabelPrefab, tracksButtonsVerticalList.transform);
			newLabel.GetComponentInChildren<Text>().text = label;
			string track_debug_string = "";
			for (int i = 0; i < (int)tracks.Count; i++)
			{
				var track = tracks[i];
				var newButton = Instantiate(trackButtonPrefab, tracksButtonsVerticalList.transform).GetComponent<Button>();
				var textMeshPro = newButton.GetComponentInChildren<Text>();

				if(type is TrackType.Audio)
				{
					track_debug_string += "\n Track Info: " + track.Name;
					// debug the values of the MediaTrack
				...
				newButton.onClick.AddListener(() => {
                    ...
                    vlcPlayer?.SetTime(vlcPlayer.Time);
                });
			}
            Debug.Log(track_debug_string);
            if (includeNone)
			{
				var newButton = Instantiate(trackButtonPrefab, tracksButtonsVerticalList.transform).GetComponent<Button>();
				var textMeshPro = newButton.GetComponentInChildren<Text>();
